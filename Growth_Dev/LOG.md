( 6-15-2026 ) Day 1 - Some Real Basic HTML Learnings - lets make a basic as hell starting point, index.html, then the later parts.

Day 2 - Short Session about a little more - colors, formating, text size, and familes.

day 3,4,5,6 - did... nothing.

day 7 - i see i can not do the set time daily goal i made, thus next week will be different. (week 0 END)

## Week 1 [for reference in log entries, the numbers mean; theory time (learning), in practice time (changes and doing), if sole number usually just practice time ]

(6-22-26)

day 1 - changed some colors and format of index.html (main-site) (30 mins)

day 2 - created an new test page and connected css file (1,17 mins)

day 3 - changed this log a little, made some cooler colors for test page (1, 20 mins)

day 4 - nothing, day was too busy for any mental power towards (0 mins)

day 5 - experimented some parts of html svg (in file) tweaks (16 mins)

day 6 - didnt do shite, thought had plans (retrospective of day 7), didnt really

day 7 - create some basic html notes (week 1 end) (6 mins)

## Week 2 [forgoing time meters for daily things] (6-29-26)

day 1 - made an html.html and thats that, will reword later

day 2 - did as above and.. a little more, converting old html.md into html.html content

day 3 - cleaned up system from project area and general ~/ area, including "Normal" (which is just a dir to slap anything that doesnt need to be in ~/)

day 4 - archieved the different repo, now both github pages repo and growth path are same repo

day 5 - LLM'd my way into some future ideas and goals to do... guys maybe in a few years ill have a bunch of.. rep.

day 6 - did whole lot more of LMM'ing my future

day 7 - didnt do much, surprisingly kinda had a life (week 2 end)

## Week 3 [back on track, now ill go just by what i did weekly henceforth] (7-6-26)
this week - did more llm'ing my future, flake'd and home-manager'd my nix file setup.  was pretty easy and didnt take much time.

## Week 4 (7-13-26) [forgot to change this, when the weekly push actually is]

started my speedrun of frontend web dev areas; completed basic html knowledge, and some css knowledge.


## Week 5 (7-20-26) [CSS week]
did quite a bit more of css.

## Week 6 (7-27-26) [Finish CSS/start JS week]
finnished css, and started js.































